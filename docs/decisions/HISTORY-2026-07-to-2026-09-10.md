...
## THREE PROJECTS READ "NOT INSTALLED" AND ALL THREE WERE PROTECTED (2026-09-09)
The Fleet page showed jev.best UNGUARDED and klima + s4biz "not installed". Measured from the
source: **all three carry perseus_client AND call add_middleware** (jev `webapp/main.py:87`, klima
`backend/app/main.py:37`, s4biz `webapp/backend/app/main.py:33`). Nothing was unprotected. Three
false negatives, two causes:
1. **A SILENT WRITE TO THE WRONG PLACE — worse than a permission error.** `BEAT_DIR` was hardcoded
   to `/var/log/colt/perseus_beats`, but `docker-compose.api.yml` mounts the shared colt_events
   volume into jev-api at **`/coltevents`** (`EVENTS_LOG=/coltevents/events.log`). That hardcoded
   path is backed by no volume there, so `os.makedirs()` **SUCCEEDED into the container's own
   ephemeral overlay**, the beat was written where nothing reads, and `perseus_beat_unwritable`
   never fired because nothing raised. jev's own traffic was visible the whole time (its Caddy
   tailer writes `service=jev-web` to the shared log), so the page read "logging but not beating" =
   UNGUARDED. FIX: derive the beat dir from `EVENTS_LOG`'s directory — the one path every project
   has already told us is on the shared volume. Same rule dbbackup and logship each learned:
   **ask where the log actually is, never assume the path.**
2. **klima and s4biz beat CORRECTLY — onto their own volumes** (polara_events, s4biz_events), which
   colt-web does not mount and must not (that `external:` coupling is what the staging gate refused;
   a status page is never worth making cybergod's deploy depend on a sibling). Their beat is
   structurally unreachable, and the page rendered that as `not installed` — **reporting OUR
   blindness as a fact about THEM**, in the sidecar column, in the very module written to prevent
   exactly that in the state column. The `elsewhere` badge was honest; the badge beside it was not.
   FIX: an own-log project reports `unverifiable`, never `not installed`, and `_beat()` now PRINTS
   the beat to stdout as `observe()` already does — the docker log driver ships it to Loki, so an
   own-volume project stays observable. The same stdout accident that made the jobhuntwow abuse
   reconstructable, used deliberately.
RULE: "the file is there" / "it is wired" / "it is beating" are three different questions, and a
page that answers the third must never phrase a blind spot as an answer to the first.
ALSO FIXED: the comment above the ROLLOUT entry for s4biz said it was "skipped explicitly" while the
line under it deployed s4biz with `--no-preview`. A comment that contradicts the code below it is
worse than none, because it is read instead of the code.
Guarded by tests/test_fleet.py: the beat dir follows the event log (jev shape), the shared-volume
projects do not regress, an explicit `PERSEUS_BEATS` still wins, the beat reaches stdout when the
directory write fails, and an own-log project is never called "not installed".
CONFIRMED ON THE LIVE PAGE: jev.best flipped to **live · active · heartbeat 4s** on the next run, so
the derived beat dir was the whole defect. cybergod/jobhuntwow/jev = 3 of 5 beating.

## 3 OF 5 WAS THE CEILING, SO THE BEAT IS NOW READ FROM stdout VIA LOKI (2026-09-09)
klima and s4biz could never appear on that page: their beat lands beside their own event log, on a
volume colt-web does not mount **and must not** -- mounting it with `external:` is what made
cybergod undeployable on staging. So the page had a permanent 3/5 ceiling and a badge that could
only ever say "we cannot tell".
THE SECOND CHANNEL WAS ALREADY THERE: `_beat()` prints the heartbeat to stdout, and this box's
promtail ships every container's stdout to the SAME Loki -- which is the substrate the perseus hub
already reads, and the same accident that made the jobhuntwow abuse reconstructable. `fleet.
_loki_beats()` asks Loki for `|= "perseus_beat"` and merges the answer.
FOUR PROPERTIES, because an optional lookup must never make a status page worse:
  * **THE FILE OUTRANKS LOKI.** colt-web writes its own beat and reads it back, so it cannot lie
    about itself; Loki is consulted ONLY for a project whose file is unreachable.
  * **A DEAD LOKI CHANGES NOTHING** -- every failure returns `{}` and the badge degrades to exactly
    the previous `unverifiable`. Absence of evidence, again.
  * **THE CACHE IS STAMPED BEFORE THE REQUEST**, so a Loki that is down costs one timeout a minute,
    not one per admin page load. A status page that hangs is its own outage.
  * **THE STATE STAYS `elsewhere`.** Only the heartbeat is proven; the traffic counts genuinely
    remain unreachable, and claiming otherwise would be the overclaim this module exists to refuse.
**THE UNPROVEN PART, STATED NOT HIDDEN:** that promtail's label scheme is not verified from here,
and this repo has already lost a week to querying a label that was never deployed
(`job="jobhuntwow"`). So three candidate selectors are tried and the first that answers wins -- the
line filter does the real work because `perseus_beat` is a rare string. If none answers, nothing
changes. `PERSEUS_LOKI_SELECTORS` overrides once the real scheme is known.
HONEST LIMIT: the Linux sandbox lost its mount, so these five tests were NOT executed by me;
`python ship.py` runs them, and the staging gate is what proves the page before production.
ALSO: `_setup` now clears `_LOKI_CACHE` and unsets LOKI_URL -- module state survives a test, so one
case's stubbed heartbeat would otherwise satisfy the next case's assertion (the colt_auth reload
pollution, one file over).
