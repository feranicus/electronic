...
# ---------- SEARCH + EXTRACTION ---------- #
async def search_linkedin(context: BrowserContext, stakeholder: Stakeholder,
                          skip_profile_visit: bool = False) -> VerificationResult:
    """Search LinkedIn for the stakeholder; return best candidate.

    skip_profile_visit: if True, only capture URL + scores from search page, don't visit profile.
                        Much safer (no authwall hits, no profile-page timeouts), but loses the
                        profile-side confidence check. Best for bulk recovery runs where we care
                        about URL capture more than precise confidence scoring.
    """
    ...
    try:
        ...
        # ---------- SKIP-PROFILE-VISIT MODE ---------- #
        # In this mode we trust the search page's best candidate and skip the profile visit.
        # This is what the scraper used to do implicitly anyway — profile visits fail often
        # due to authwall/"join LinkedIn to view" for 2nd/3rd-degree connections, and when
        # they fail, confidence drops to 0 even though the URL is usually correct.
        if skip_profile_visit:
            # Use search-snippet score as the confidence; classify based on that
            result.linkedin_url = profile_url
            result.matched_name = best["anchor_text"][:100]
            result.matched_title = best["container_text"][:200]
            result.confidence = round(best_score, 3)
            if best_score >= 0.70:
                result.status = "verified"
                result.notes = f"search-snippet only (profile visit skipped). Score {best_score:.2f}."
            elif best_score >= 0.50:
                result.status = "candidate"
                result.notes = f"search-snippet only (profile visit skipped). Moderate score {best_score:.2f} — spot-check before outreach."
            else:
                result.status = "candidate"
                result.notes = f"search-snippet only. Low score {best_score:.2f} — likely wrong person, verify manually."
            return result
...
async def search_google_for_linkedin(context: BrowserContext, stakeholder: Stakeholder) -> VerificationResult:
    ...
    try:
        ...
        for attempt, query in enumerate(queries):
            ...
            # ---------- GOOGLE CONSENT + CAPTCHA HANDLING ---------- #
            # Google EU shows cookie consent. Google sometimes shows captcha / "unusual traffic".
            # We MUST distinguish: captcha = pause-for-human, consent = auto-click.
            ...
            if "sorry/index" in current_url or "sorry.google.com" in current_url:
                is_captcha = True
            elif any(x in page_title for x in ["captcha", "not a robot", "verify you're", "verify you are"]):
                is_captcha = True
            else:
                # Check for captcha iframe or "unusual traffic" text (narrow scope)
                try:
                    captcha_iframe = await page.query_selector("iframe[src*='recaptcha'], iframe[src*='captcha']")
                    if captcha_iframe:
                        is_captcha = True
                    else:
                        # Look for the specific 'unusual traffic' phrase only in h1/h2 (not page footers etc.)
                        heading = await page.query_selector("h1, h2")
                        if heading:
                            h_text = (await heading.inner_text()).lower()
                            if "unusual traffic" in h_text or "before you continue" in h_text and "captcha" in h_text:
                                is_captcha = True
                except Exception:
                    pass

            if is_captcha:
                log(f"  [GOOGLE-CAPTCHA] detected for '{stakeholder.full_name}'.")
                log(f"  [PAUSE] Solve the captcha manually in the open browser window,")
                log(f"  [PAUSE] then the script will retry this profile automatically.")
                # Wait up to 10 min for human to solve; poll every 15s to auto-continue
                waited = 0
                while waited < 600:
                    ...
                    try:
                        ...
                        if "sorry" not in now_url and "captcha" not in (await page.title()).lower():
                            log(f"  [OK] Captcha resolved after {waited}s. Retrying this profile...")
                            break
                    except Exception:
                        continue
                else:
                    log(f"  [TIMEOUT] Captcha not solved within 10 min. Marking as error, will retry later.")
                    result.status = "error"
                    result.notes = "Google captcha — not solved within timeout"
                    return result

                # Re-navigate to the search URL after captcha solve
                try:
                    await page.goto(url, wait_until="domcontentloaded", timeout=30000)
                    await asyncio.sleep(random.uniform(2.0, 3.5))
                except Exception:
            ...
            # Google EU cookie consent — auto-dismiss (NOT captcha)
            try:
                # Most common selectors for Google's consent page
                consent_selectors = [
                    "#L2AGLb",                                    # "I agree" button (old)
...
async def run(input_xlsx: Path, output_xlsx: Path,
              ...
              google_fallback: bool = False, skip_profile_visit: bool = False):
            ...
            with Progress(SpinnerColumn(), TextColumn("{task.description}"),
                ...
                for i, s in enumerate(stakeholders):
                    ...
                    if google_fallback:
                        ...
                        result = await search_linkedin(context, s, skip_profile_visit=skip_profile_visit)
                    ...
                    # On Windows, if the user has verified.xlsx open in Excel, it's locked for writing.
                    # In that case, write to a .tmp.xlsx sibling so progress isn't lost.
                    try:
                        write_output(stakeholders, checkpoint, output_xlsx)
                    except PermissionError:
                        # Excel has the file open. Write to a .tmp.xlsx instead.
                        tmp_path = output_xlsx.with_name(output_xlsx.stem + ".tmp" + output_xlsx.suffix)
                        try:
                            write_output(stakeholders, checkpoint, tmp_path)
                            # Only log once per session to reduce noise
                            if not getattr(run, "_tmp_warned", False):
                                log(f"[INFO] '{output_xlsx.name}' is locked (likely open in Excel). "
                                    f"Writing progress to '{tmp_path.name}' instead. Close Excel to resume normal writes.")
                                run._tmp_warned = True  # type: ignore
                        except Exception as e2:
                            log(f"[WARN] Could not write even tmp Excel: {e2}")
                    except Exception as e:
...
def main():
    ...
    p.add_argument("--skip-profile-visit", action="store_true",
                   help="In LinkedIn mode: capture URL + score from search page only, don't visit profile. Safer (no authwall hits) and faster. Use when rescuing from lost URLs or for bulk recovery. Confidence is based on search-snippet score only.")
    ...
    asyncio.run(run(args.input, args.output,
                    ...
                    skip_profile_visit=args.skip_profile_visit))
