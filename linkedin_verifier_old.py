...
async def detect_challenge(page: Page) -> bool:
    ...
    try:
        ...
        if any(x in content.lower() for x in ["security check", "verify you're human", "captcha", "unusual activity"]):
            return True
    except Exception:
        pass
    return False
...
async def search_linkedin(context: BrowserContext, stakeholder: Stakeholder) -> VerificationResult:
    ...
    try:
        ...
        # Collect the first public profile link on the page.
        selectors = [
            "a.app-aware-link[href*='/in/']",                       # 2024-2025 style
            "div.search-results-container li a[href*='/in/']",     # older
            "a[href*='/in/'][data-test-app-aware-link]",           # alt
            "span.entity-result__title-text a[href*='/in/']",      # entity cards
        ...
        for sel in selectors:
            try:
                first_card = await page.query_selector(sel)
                if first_card:
                    href_test = await first_card.get_attribute("href")
                    if href_test and "/in/" in href_test:
                        log(f"  selector matched: {sel}")
                        break
                    first_card = None
            except Exception:
                continue
        ...
        if not first_card:
            ...
            result.notes = "No results in people search (all selectors failed)."
        ...
        # Optionally visit the profile to pull current title/company for confidence scoring
        await human_delay()
        ...
        if confidence >= 0.70:
            ...
            result.notes = "Moderate match — needs manual confirmation."
        else:
            result.status = "candidate"
            result.notes = "Low match — likely a false positive."
...
# ---------- MAIN PIPELINE ---------- #
async def run(input_xlsx: Path, output_xlsx: Path, test_n: Optional[int] = None):
        ...
        try:
            with Progress(SpinnerColumn(), TextColumn("{task.description}"),
                ...
                for i, s in enumerate(stakeholders):
                    ...
                    if key in checkpoint and checkpoint[key].get("status") == "verified":
                        log(f"[SKIP] {s.full_name} — already verified.")
...
def main():
    ...
    p.add_argument("--reset", action="store_true", help="Reset checkpoint and start over")
    args = p.parse_args()
    ...
    asyncio.run(run(args.input, args.output, test_n=args.test))
