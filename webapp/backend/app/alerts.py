...
def observe_http(ev):
    ...
    if _count("req:all", DDOS_WIN) >= DDOS_REQ_N:
        ...
        if len(ips) >= DDOS_IP_N:
            fire("ddos", "global", "Possible DDoS / traffic flood on cybergod.ai",
    ...
    if status == 404:
        ...
        if _count("404:%s" % ip, PROBE_404_WIN) >= PROBE_404_N:
            fire("dir_bruteforce", ip, "Directory brute-force against cybergod.ai",
...
def observe_login_failure(email, ip, reason, ua=""):
    ...
    if n >= FAIL_LOGIN_N:
        fire("login_failed", email or ip, "Repeated failed logins on cybergod.ai",
